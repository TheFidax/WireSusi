...
int8_t sendCommandWireSusi(uint8_t I2C_Address, uint8_t FirstByte, uint8_t SecondByte) {                // Provo a mandare un comando RCN600 ad uno Slave I2c
...
int16_t readCVsWireSusi(uint8_t I2C_Address, uint16_t cvAddress) {		                                // Provo a richiedere la lettura CV da uno Slave I2c
...
int16_t writeCVsWireSusi(uint8_t I2C_Address, uint16_t cvAddress, uint8_t cvValue) {                    // Provo a scrivere una CV in uno Slave
    uint8_t error;															                            // Risultato del metodo Wire.endTransmission
    interrupts();															                            // Abilito gli Interrupt per la Gestione I2c
    Wire.beginTransmission(I2C_Address);									                            // Inizio la trasmissione verso lo Slave interessato
...
Rcn600Message* searchFreeMessage(void) {                                                                // Cerca uno Slot libero dove salvare i dati in arrivo
    for (uint8_t i = 0; i < SUSI_BUFFER_LENGTH; ++i) {                                                  // Scorro tutta la lunghezza del Buffer
        if (_Buffer[i].nextMessage == FREE_MESSAGE_SLOT) {                                              // Se uno e' contrassegnato come "FREE" ("LIBERO")
            return &_Buffer[i];                                                                         // Restituisco l'indirizzo di quello Slot
        }
        //else{}                                                                                        // In caso negativo continuo a cercare
    }

    return NULL;                                                                                        // Se non ci sono SLot liberi restituisco NULL
}
